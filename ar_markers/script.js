...
let ARMarker = {
  ...
  initGLForARMarkerDetection: function(gl, width, height, texture_unit_base = 0,
    ...
    this.createFloatTexture = (gl, w, h) => {
      gl.activeTexture(gl[`TEXTURE${texture_unit}`]);
      const texture = gl.createTexture();
      gl.bindTexture(gl.TEXTURE_2D, texture);
      gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE);
    ...
    const corners_by5_texture = this.createFloatTexture(gl, width / 5, height / 5);
    const edges_by5_texture = this.createFloatTexture(gl, width / 5, height / 5);
    const squares_by40_texture = this.createFloatTexture(gl, width / 40, height / 30);
    ...
    gl.passes = [{
      in: texture,
      framebuffer: this.createFramebuffer2D(gl, [threshold_texture]),
      program: this.createProgram(gl, threshold_vertex, threshold_pixel, texture)
    }, {
      in: threshold_texture,
      framebuffer: this.createFramebuffer2D(gl, [corners_texture]),
      program: this.createProgram(gl, corners_vertex, corners_pixel, threshold_texture)
    }, {
      in: corners_texture,
      out: [corners_refine_texture],
      framebuffer: this.createFramebuffer2D(gl, [corners_refine_texture]),
      program: this.createProgram(gl, corners_refine_vertex, corners_refine_pixel,
                             corners_texture)
    }, {
      in: corners_refine_texture,
      out: [corners_by5_texture],
      framebuffer: this.createFramebuffer2D(gl, [corners_by5_texture]),
      program: this.createProgram(gl, corners_by5_vertex, corners_by5_pixel,
                             corners_refine_texture)
    }, {
      in: corners_by5_texture,
      out: [edges_by5_texture],
      framebuffer: this.createFramebuffer2D(gl, [edges_by5_texture]),
      program: this.createProgram(gl, edges_by5_vertex, edges_by5_pixel,
                             corners_by5_texture)
    }, {
      in: edges_by5_texture,
      out: [squares_by40_texture],
      framebuffer: this.createFramebuffer2D(gl, [squares_by40_texture]),
      program: this.createProgram(gl, squares_by40_vertex, squares_by40_pixel,
                             edges_by5_texture)
    }, {
      in: texture,
      framebuffer: null,
      program: this.createProgram(gl, simple_vertex, simple_pixel, texture)
    }, {
      in: squares_by40_texture,
      framebuffer: null,
      outlines: (width / 40) * (height / 30), // how many squares to draw.
      program: this.createProgram(gl, lineloop_vertex, lineloop_pixel, squares_by40_texture)
    }, {
      in: squares_by40_texture,
      framebuffer: null,
      codes: (width / 40) * (height / 30), // how many codes to draw.
      program: this.createProgram(gl, codelabel_vertex, codelabel_pixel, squares_by40_texture)
    }];
